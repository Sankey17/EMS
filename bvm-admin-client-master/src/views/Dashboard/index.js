...
import { getTimeInfo, updateWatch, getAllMovementByYear } from "../../actions";
...
class Dashboard extends Component {
    ...
    if (movements.success && movements.data && movements.data.length) {
      for (i = 0; i < movements.data.length; i++) {
        ...
        if (movements.data && movements.data[i] && movements.data[i].date) {
          ...
          const seconds = ("0" + (Math.floor(workingTime / 1000) % 60)).slice(
            -2
          );
          const minutes =
            ("0" + (Math.floor(workingTime / 60000) % 60)).slice(-2) || 0;
          const hours =
            ("0" + Math.floor(workingTime / 3600000)).slice(-2) || 0;
  ...
  submitWatch = async (key, status) => {
    ...
    const { todayTimestamp, breakStart, lunchStart, workStart } = this.state;
      ...
      <div className="animated fadeIn">
        <Row className="justify-content-center text-center">
          <Col
            md="4"
            style={{
              pointerEvents: stopEvent ? "unSet" : "none",
              opacity: stopEvent ? 1 : 0.65,
            }}
          >
            <Stopwatch
          ...
          <Col
            md="4"
            style={{
              pointerEvents: lunchEvent ? "unSet" : "none",
              opacity: lunchEvent ? 1 : 0.65,
            }}
          >
            <LunchWatch
              lunchStart={lunchStart}
          ...
          <Col
            md="4"
            style={{
              pointerEvents: breakEvent ? "unSet" : "none",
              opacity: breakEvent ? 1 : 0.65,
            }}
          >
            <BreakWatch
